                    ...
                    <tr>
                        ...
                        <td>
                            <form action="/my-first-blog/admin/users/changerole/<?= $user->id ?>" method="POST">
                                <select class="form-select" style="min-width: 160px;" aria-label="role select" name="is_admin" <?= $user->id === $_SESSION['user_id'] ? 'disabled' : '' ?> onchange="this.form.submit()"> <!-- soumission du formulaire dès qu'une nouvelle option est sélectionnée -->
